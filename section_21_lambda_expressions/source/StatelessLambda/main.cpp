...
void test2()
{
    ...
    auto l3 = [](int &x, int y)
    {
        ...
        x = 1000; // actual value is going to be changed - because of reference
        y = 2000; // only local variable would change
    };
...
void test3()
{
    ...
    l5(stooge); // copy constructor is going to be called (of class Person)

    auto l6 = [](Person &p) { // reference so we can modify the real object
        p.set_name("Frank");
        p.set_age(25);
        std::cout << p << std::endl;
    };
    l6(stooge); // alias no copy
