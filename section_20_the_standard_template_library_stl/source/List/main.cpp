...
void test3()
{
    ...
    persons.resize(5); // uses the Person default constructor - if we would not define it, it would throw error
    display(persons);
}
void test4()
{
...
void test5()
{
    ...
    auto it = std::find(stooges.begin(), stooges.end(), Person{"Moe", 25}); // will use == operator
    if (it != stooges.end())
        stooges.emplace(it, "Frank", 18); // before iterator
    display(stooges);
}
