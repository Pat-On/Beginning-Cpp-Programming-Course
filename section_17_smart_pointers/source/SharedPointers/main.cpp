...
int main()
{
    ...
    std::shared_ptr<Test> ptr = std::make_shared<Test>(100); // 1
    
    func(ptr);
    std::cout << "Use count L-48: " << ptr.use_count() << std::endl;
    {
        std::shared_ptr<Test> ptr1 = ptr; // 2
        std::cout << "Use count L-51: " << ptr.use_count() << std::endl;
        {
            std::shared_ptr<Test> ptr2 = ptr; // 3
            std::cout << "Use count L-54: " << ptr.use_count() << std::endl;
            ptr.reset();
        }
        std::cout << "Use count L-57: " << ptr.use_count() << std::endl; // 0
    }
    std::cout << "Use count L-59: " << ptr.use_count() << std::endl; // 
