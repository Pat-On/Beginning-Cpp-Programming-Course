...
void test1()
{
    ...
    std::cout << "\nDirection " << direction << std::endl; // passing direction to overloaded operator
    ...
    direction = static_cast<Direction>(100);               // code from line 61 is very similar to this one
    std::cout << "\nDirection " << direction << std::endl; // Displays 100!
    std::cout << direction_to_string(direction) << std::endl;
}
...
// paramater is a valid enumerator or not.
...
std::istream &operator>>(std::istream &is, State &state)
{
    ...
    std::underlying_type_t<State> user_input; // better is to use underlying_type_t, because it is more resilient for future changes
    is >> user_input;
